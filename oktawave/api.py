...
class OktawaveApi:
	...
	def _init_common(self, args):
		...
		if hasattr(self, 'common'):
			return
		self.common = ApiClient(wsdl_common, args.username, args.password, hc_common, docs_common, args.debug)
		self._d(self.common)
	def _init_clients(self, args):
		"""Convenience method to initialize ClientsService client"""
		if hasattr(self, 'clients'):
			return
		self.clients = ApiClient(wsdl_clients, args.username, args.password, hc_clients, docs_clients, args.debug)
	...
	def _search_params(self, sp):
		# getting rid of NoneType non-special attributes, required for searchParams
		self._d(sp)
	...
	def _find_disk(self, disk_id):
		...
		disks = [d for d in self.clients.call('GetDisks', dsp)._results[0] if d.ClientHddId == disk_id]
		if len(disks) == 0:
			return None
		res = disks[0]
	...
	def Account_Settings(self, args):
		...
		res['Time zone'] = [client._x003C_TimeZone_x003E_k__BackingField.DisplayName]
		res['Currency'] = [self._dict_item_name(client._x003C_Currency_x003E_k__BackingField)]
		res['Date format'] = [self._dict_item_name(client._x003C_DateFormat_x003E_k__BackingField)]
		res['Availability zone'] = [self._dict_item_name(self.common.call('GetDictionaryItemById', client._x003C_AvailabilityZone_x003E_k__BackingField))]
		res['24h clock'] = ['Yes' if client._x003C_Is24HourClock_x003E_k__BackingField else 'No']
	...
	def OCI_List(self, args):
		...
		self._logon(args)
		sp = self._search_params(self.clients.create('ns3:VirtualMachineSearchParams'))
	...
	def OCI_Logs(self, args):
		...
		self._logon(args)
		sp = self._search_params(self.clients.create('ns3:VirtualMachineHistoriesSearchParams'))
		sp.VirtualMachineId = args.id
		sp.PageSize = 100
		sp.SortingDirection = 'Descending'
		data = self.clients.call('GetVirtualMachineHistories', sp, self.client_id)
		self._d(data)
	...
	def OCI_Settings(self, args):
		...
		self._logon(args)
		data = self.clients.call('GetVirtualMachineById', args.id, self.client_id)
	...
	def OCS_Get(self, args):
		...
		if args.path == None:
			...
			self.p.print_swift_file(self.sc.get_object(args.container, args.path))
	def OCS_List(self, args):
		"""Lists content of a directory or container"""
		self._ocs_prepare(args)
		obj = self.sc.get_container(args.container)	# TODO: perhaps we can optimize it not to download the whole container when not necessary
	...
	def OCS_CreateDirectory(self, args):
		...
		self._ocs_prepare(args)
		self.sc.put_object(args.container, args.path, None, content_type='application/directory')
		print "OK"
	...
	def OVS_List(self, args):
		...
		self.p.print_table([['ID', 'Name', 'Tier', 'Capacity', 'Used', 'Shared', 'VMs']] + [[
			...
			"None" if disk.VirtualMachineHdds == None else ', '.join([str(hdd.VirtualMachine.VirtualMachineId) + " (" + hdd.VirtualMachine.VirtualMachineName + ")" for hdd in disk.VirtualMachineHdds[0]])
	...
	def OVS_Map(self, args):
		...
		if disk == None:
			...
			return 1
		vms = [vm.VirtualMachine.VirtualMachineId for vm in disk.VirtualMachineHdds[0]]
	...
	def ORDB_List(self, args):
		"""Lists databases"""
		self._logon(args)
		sp = self._search_params(self.clients.create('ns3:DatabaseInstanceSearchParams'))
		sp.ClientId = self.client_id
	...
	def ORDB_Delete(self, args):
		...
		if args.db_name == None:
			...
			self.clients.call('DeleteDatabase', args.id, args.db_name, self.client_id)
	...
	def ORDB_Create(self, args):
		...
		if str(data.TemplateType.DictionaryItemId) != str(DICT['DB_VM_CATEGORY']):
			...
			return 1
		self.OCI_Create(args, forced_type = 'Database', db_type = data.DatabaseType.DictionaryItemId)
	...
	def ORDB_CreateLogicalDatabase(self, args):
		...
		self._logon(args)
		self.clients.call('CreateDatabase', args.id, args.name, DICT[args.encoding.upper() + '_ENCODING'], self.client_id)
		print "OK"
...
#		print "Not implemented yet"
	...
	def ORDB_MoveLogicalDatabase(self, args):
		...
		self._logon(args)
		self.clients.call('MoveDatabase', args.id_from, args.id_to, args.name, self.client_id)
		print "OK"
	...
	def ORDB_RestoreLogicalDatabase(self, args):
		...
		self._logon(args)
		self.clients.call('RestoreDatabase', args.id, args.name, args.backup_file, self.client_id)
		print "OK"	
...
#				continue
#			for item in data[0]:
#				print "  " + str(item.DictionaryItemId) + "  " + self._dict_item_name(item)
